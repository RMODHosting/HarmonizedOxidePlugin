...
if (Test-Path -Path uMod$tag.zip) {
    Write-Host uMod $tag is already up to date.
} else {
    Remove-Item .\uMod*.zip -Force
    Write-Host Downloading uMod $tag . . .
    Invoke-WebRequest https://umod.org/games/rust/download/develop -Out .\uMod$tag.zip
    Write-Host Extracting uMod $tag . . .
    Expand-Archive -DestinationPath .\raw-deps\linux\ .\uMod$tag.zip -Force
}